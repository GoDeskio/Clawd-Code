...
**Версия 0.2.1 — Jonathan Ai.** Десктопное приложение само является агентом. Достаточно локальной модели или одного API-ключа. MCP, Cursor и Codex необязательны. Переименовывайте чаты в боковой панели. Схемы инструментов отправляются в классическом виде Anthropic (`input_schema.type`). При 400 schema тот же запрос повторяется без tools.
