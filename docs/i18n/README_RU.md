...
**Версия 0.2.0 — Jonathan Ai.** Десктопное приложение само является агентом. Достаточно локальной модели или одного API-ключа. MCP, Cursor и Codex необязательны. Переименовывайте чаты в боковой панели. Схемы инструментов очищаются (`input_schema.type`), чтобы избежать Anthropic 400.
