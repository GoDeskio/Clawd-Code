...
def _rss(url: str, limit: int) -> dict[str, Any]:
    ...
    parsed = feedparser.parse(url, request_headers={"User-Agent": "JonathanAi/0.4.9"})
...
def _youtube(url: str, language: str) -> dict[str, Any]:
    ...
    if preferred and preferred.get("url"):
        request = urllib.request.Request(str(preferred["url"]), headers={"User-Agent": "Mozilla/5.0 JonathanAi/0.4.9"})
