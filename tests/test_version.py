"""Product version is 0.2.6 and is referenced from docs and packaging."""
...
class TestVersion(unittest.TestCase):
    def test_version_file_and_package(self) -> None:
        self.assertEqual((ROOT / "VERSION").read_text(encoding="utf-8").strip(), "0.2.6")
        self.assertEqual(get_version(), "0.2.6")
    ...
    def test_docs_and_installer_mention_version(self) -> None:
        ...
        self.assertIn("0.2.6", readme)
        self.assertIn("0.2.6", win)
        self.assertIn("0.2.6", iss)
        self.assertIn("0.2.6", html)
        self.assertIn("0.2.6", setup_c)
