...
class TestWindowsArtifacts(unittest.TestCase):
    ...
    def test_wizard_copy_has_next_install_finish(self) -> None:
        ...
        self.assertIn(".cache\\install\\latest.log", nsis)
...
class TestDashboardHtml(unittest.TestCase):
    def test_glass_dashboard_and_conversation_list(self) -> None:
        ...
        self.assertIn('title: "Jonathan Ai 0.4.9"', main_js)
        self.assertIn('"window_created"', main_js)
        self.assertIn('"window_load_failed"', main_js)
        self.assertIn("setTimeout(() => app.quit(), 2500)", main_js)
        self.assertIn('"clawd:installUpdate"', main_js)
        self.assertIn('"clawd:restartApp"', main_js)
        ...
        self.assertIn("installUpdate", preload_js)
        ...
        self.assertIn("/api/jobs/events?job_id=", app_js)
        self.assertIn("void pollJob()", app_js)
        self.assertIn("sessionsRequestId", app_js)
        self.assertIn("creatingSession", app_js)
        self.assertIn("checkForUpdates", app_js)
